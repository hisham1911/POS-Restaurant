        ...
        <Card>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFilterChange(
                      "type",
                      (e.target.value ||
                        undefined) as CashRegisterFilters["type"],
                    )
                  }
            ...
            <ShiftIdFilter
              ...
              onChange={(shiftId) => handleFilterChange("shiftId", shiftId)}
            />
          </div>
        </Card>
        ...
        >
          ...
          {shifts.map((shift) => (
            <option key={shift.id} value={shift.id}>
              وردية #{shift.id} — {shift.userName}
              {shift.isClosed ? " (مغلقة)" : " (مفتوحة)"}
            </option>
          ))}
        </select>
        <ChevronDown className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
