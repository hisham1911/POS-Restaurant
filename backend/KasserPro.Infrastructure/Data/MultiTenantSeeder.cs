...
public static class MultiTenantSeeder
{
    ...
    private static async Task SeedHomeAppliancesStoreAsync(AppDbContext context)
    {
        ...
        {
            // أدوات مطبخ
            new() { TenantId = tenant.Id, CategoryId = categories[0].Id, Name = "طقم سكاكين", NameEn = "Knife Set", Sku = "KIT001", Price = 150, Cost = 80, AverageCost = 80, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            new() { TenantId = tenant.Id, CategoryId = categories[0].Id, Name = "مقشرة خضار", NameEn = "Vegetable Peeler", Sku = "KIT002", Price = 25, Cost = 12, AverageCost = 12, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            
            // أجهزة كهربائية
            new() { TenantId = tenant.Id, CategoryId = categories[1].Id, Name = "خلاط كهربائي", NameEn = "Electric Blender", Sku = "ELC001", Price = 450, Cost = 280, AverageCost = 280, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            new() { TenantId = tenant.Id, CategoryId = categories[1].Id, Name = "محمصة خبز", NameEn = "Toaster", Sku = "ELC002", Price = 320, Cost = 200, AverageCost = 200, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            
            // أواني وأطباق
            new() { TenantId = tenant.Id, CategoryId = categories[2].Id, Name = "طقم أطباق", NameEn = "Dish Set", Sku = "DSH001", Price = 280, Cost = 150, AverageCost = 150, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true }
        };
    ...
    private static async Task SeedRestaurantAsync(AppDbContext context)
    {
        ...
        var products = new List<Product>
        {
            new() { TenantId = tenant.Id, CategoryId = categories[0].Id, Name = "كباب", NameEn = "Kebab", Sku = "GRL001", Price = 80, Cost = 45, AverageCost = 45, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            new() { TenantId = tenant.Id, CategoryId = categories[0].Id, Name = "كفتة", NameEn = "Kofta", Sku = "GRL002", Price = 70, Cost = 40, AverageCost = 40, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            new() { TenantId = tenant.Id, CategoryId = categories[1].Id, Name = "سلطة", NameEn = "Salad", Sku = "APP001", Price = 25, Cost = 10, AverageCost = 10, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true },
            new() { TenantId = tenant.Id, CategoryId = categories[2].Id, Name = "عصير طازج", NameEn = "Fresh Juice", Sku = "DRK001", Price = 30, Cost = 15, AverageCost = 15, TaxRate = 14, TaxInclusive = false, TrackInventory = true, IsActive = true }
        };
        context.Products.AddRange(products);
        await context.SaveChangesAsync();
